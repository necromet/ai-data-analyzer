...
AXIS TYPE (x_axis_type / y_axis_type):
Optional field accepted by all charts with x/y axes. Valid values:
- "category" (default for category/label axes) — discrete string categories
- "value" (default for numeric axes) — continuous numeric scale
- "time" — time series with automatic date/time formatting
- "log" — logarithmic scale, useful for data spanning multiple orders of magnitude
Omit if the default is correct. Use "time" when the axis contains dates/timestamps. Use "log" when values vary by orders of magnitude.
